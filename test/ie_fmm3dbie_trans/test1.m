function [varargout] =  test1(npu, norder, occ, zk, rank_or_tol, m)

%%%%% a basic test code for constructing operators for transmission 
%%%%% problems. Not supposed to be used more generally.
...
[u0,du0] = helm_transmission_kernel(x_or, xyz_in, zk0, nu);
B1 =  (u0*q).*sqrt(area).';
B2 = -(du0*q/deps0).*sqrt(area).';
